...
/// Provides sandbox order execution via the T-Invest gRPC SandboxService.
///
/// Implements the [`ExecutionClient`] trait using the T-Invest Sandbox
/// gRPC service with [`ExecutionClientCore`] for identity/state and
/// [`ExecutionEventEmitter`] for async event dispatch.
#[derive(Debug)]
pub struct TInvestSandboxExecutionClient {
...
impl TInvestSandboxExecutionClient {
    /// Creates a new [`TInvestSandboxExecutionClient`].
    #[must_use]
    pub fn new(
        trader_id: TraderId,
        client_id: ClientId,
    ...
    {
        let span = info_span!("tinvest_sandbox_exec", task = description);
    ...
    pub async fn sandbox_pay_in(&self, currency: &str, amount: f64) -> anyhow::Result<()> {
        ...
        let request = crate::proto::SandboxPayInRequest {
            account_id: self.core.account_id.to_string(),
            amount: Some(crate::proto::MoneyValue {
                units: amount.trunc() as i64,
                nano: (amount.fract() * 1_000_000_000.0).round() as i32,
                currency: currency.to_string(),
            }),
        };

        let request = self.grpc_client.with_auth(tonic::Request::new(request));
        stub.sandbox_pay_in(request).await?;
        info!("Sandbox pay in: {amount} {currency}");
        Ok(())
    }
    ...
    pub async fn get_sandbox_portfolio(
        ...
        let request = self.grpc_client.with_auth(tonic::Request::new(request));
        let response = stub.get_sandbox_portfolio(request).await?;
        let portfolio = response.into_inner();
        info!(
            "Got sandbox portfolio: {} positions",
    ...
    pub async fn get_sandbox_positions(
        ...
        let request = self.grpc_client.with_auth(tonic::Request::new(request));
        let response = stub.get_sandbox_positions(request).await?;
        let positions = response.into_inner();
        info!(
            "Got sandbox positions: {} securities, {} futures, {} options",
            positions.securities.len(),
            positions.futures.len(),
            positions.options.len(),
        );
        Ok(Some(positions))
    }
    ...
    pub async fn sandbox_get_last_prices(
        ...
        info!("sandbox_get_last_prices: {} instruments", figi.len());
        ...
        info!("Sandbox: got {} last prices", resp.last_prices.len());
        Ok(resp)
    }

    /// Get portfolio for the given account via OperationsService.
    pub async fn sandbox_get_portfolio(
        &self,
        account_id: &str,
        ...
        info!("sandbox_get_portfolio: account={account_id}");
        ...
        let resp = response.into_inner();
        info!(
            "Sandbox portfolio for {}: {} positions",
            account_id,
            resp.positions.len(),
        );
        Ok(resp)
    ...
    /// Get operations for the given account via OperationsService.
    #[allow(deprecated)]
    pub async fn sandbox_get_operations(
        &self,
        account_id: &str,
        figi: &Option<String>,
        ...
        info!("sandbox_get_operations: account={account_id}");
    ...
    /// Fetch the sandbox portfolio from the API and emit an [`AccountState`] event.
    pub fn update_account_state(&self) {
        ...
        self.spawn_task("sandbox_update_account_state", async move {
            ...
            info!("Sandbox account state emitted for {account_id_str}");
            Ok(())
        });
    }
}
...
#[async_trait(?Send)]
impl ExecutionClient for TInvestSandboxExecutionClient {
    ...
    fn start(&mut self) -> anyhow::Result<()> {
        info!("Starting T-Invest Sandbox execution client");
    ...
    fn stop(&mut self) -> anyhow::Result<()> {
        info!("Stopping T-Invest Sandbox execution client");
    ...
    async fn connect(&mut self) -> anyhow::Result<()> {
        ...
        info!("Connected T-Invest Sandbox execution client");
        Ok(())
    }
    ...
    async fn disconnect(&mut self) -> anyhow::Result<()> {
        ...
        info!("Disconnected T-Invest Sandbox execution client");
        Ok(())
    }
    ...
    fn submit_order(&self, cmd: SubmitOrder) -> anyhow::Result<()> {
        ...
        info!(
            "Sandbox submit order: figi={}, side={}, qty={}, type={}, client_oid={}",
            figi, side, quantity, order_type, client_order_id,
        );

        self.spawn_task("sandbox_submit_order", async move {
            let mut stub = grpc_client.sandbox().await?;
            ...
            match stub.post_sandbox_order(request).await {
                Ok(response) => {
                    let resp = response.into_inner();
                    let venue_order_id = VenueOrderId::new(&resp.order_id);
                    info!(
                        "Sandbox order submitted: order_id={}, status={}",
                        resp.order_id,
                ...
                Err(e) => {
                    error!("Failed to submit sandbox order: {e} (instrument={figi}, client_oid={client_order_id})");
    ...
    fn submit_order_list(&self, cmd: SubmitOrderList) -> anyhow::Result<()> {
        debug!("sandbox submit_order_list called: {cmd:?}");
        Ok(())
    }
    ...
    fn modify_order(&self, cmd: ModifyOrder) -> anyhow::Result<()> {
        ...
        info!(
            "Sandbox modify order: venue_order_id={}, qty={}",
            venue_order_id, quantity,
        );

        self.spawn_task("sandbox_modify_order", async move {
            let mut stub = grpc_client.sandbox().await?;
            ...
            match stub.replace_sandbox_order(request).await {
                Ok(response) => {
                    let resp = response.into_inner();
                ...
                Err(e) => {
                    error!("Failed to modify sandbox order: {e} (venue_oid={venue_order_id})");
    ...
    fn batch_modify_orders(&self, cmd: BatchModifyOrders) -> anyhow::Result<()> {
        debug!("sandbox batch_modify_orders called: {cmd:?}");
        Ok(())
    }
    ...
    fn cancel_order(&self, cmd: CancelOrder) -> anyhow::Result<()> {
        ...
        info!("Sandbox cancel order: venue_order_id={}", venue_order_id);

        self.spawn_task("sandbox_cancel_order", async move {
            let mut stub = grpc_client.sandbox().await?;
            ...
            match stub.cancel_sandbox_order(request).await {
                Ok(_) => {
                    info!("Sandbox order cancelled: {}", venue_order_id);
                    emitter.emit_order_canceled(&order, Some(venue_oid), ts_event);
                }
                Err(e) => {
                    error!("Failed to cancel sandbox order: {e} (venue_oid={venue_order_id})");
    ...
    fn cancel_all_orders(&self, cmd: CancelAllOrders) -> anyhow::Result<()> {
        debug!("sandbox cancel_all_orders called: {cmd:?}");
        Ok(())
    }

    fn batch_cancel_orders(&self, cmd: BatchCancelOrders) -> anyhow::Result<()> {
        debug!("sandbox batch_cancel_orders called: {cmd:?}");
        Ok(())
    }
    ...
    fn query_order(&self, cmd: QueryOrder) -> anyhow::Result<()> {
        debug!("sandbox query_order called: {cmd:?}");
        Ok(())
    }
    ...
    async fn generate_order_status_report(
        ...
        info!("sandbox generate_order_status_report: account={account_id_str}");
        ...
        match stub.get_sandbox_order_state(request).await {
            Ok(response) => {
            ...
            Err(e) => {
                error!("Sandbox get_order_state failed: {e}");
                Err(e.into())
            }
        }
    }
    ...
    async fn generate_order_status_reports(
        ...
        info!("sandbox generate_order_status_reports: account={account_id_str}");
        ...
        let response = stub.get_sandbox_orders(request).await?;
    ...
    async fn generate_fill_reports(
        ...
        info!("sandbox generate_fill_reports: account={account_id_str}");
        ...
        loop {
            ...
            match stub.get_sandbox_operations_by_cursor(request).await {
                Ok(response) => {
                    let resp = response.into_inner();
                ...
                Err(e) => {
                    error!("Sandbox get_operations failed: {e}");
                    return Err(e.into());
                }
            }
        }
    ...
    async fn generate_position_status_reports(
        ...
        info!("sandbox generate_position_status_reports: account={account_id_str}");
    ...
    async fn generate_mass_status(
        ...
        info!("sandbox generate_mass_status: account={account_id_str}");
        ...
        info!(
            "Sandbox mass status: {} orders, {} fills, {} positions",
